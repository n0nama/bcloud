(function($) {
	...
	$(document).ready(function() {
		...
		$('#adminbar .wp-menu-top-first a').click(function(e) {
			e.preventDefault();
		    var url = $(this).attr('href');
			window.open(url, '_blank');
		});
		$('#adminmenu .wp-no-url').click(function(){
			$('#adminmenu li').each(function(){
				$(this).removeClass('wp-menu-open');
			});
			$(this).toggleClass('wp-menu-open');
			$(this).children('.wp-has-submenu').hover(function(e){
				e.preventDefault();
				$(this).children('.wp-submenu').show();
			});
		});
		
		$('#actions').click(function(){
			$('#content-related').toggle();
		});

		// left submenus should stay on page
		/*$('#adminmenu .wp-has-submenu a').click(
			function() {
		...
		$('#adminbar .wp-has-submenu a').click(
			function() {
				/*var $submenu = $(this).children('.wp-submenu').css({'visibility': 'hidden', 'display': 'block'}),
					extra_margin = 0,
