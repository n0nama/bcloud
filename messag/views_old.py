...
class Inbox(View, LoginRequiredMixin):
    def get(self, request):
        template = 'messages/inbox_react.html'
        return render_to_response(template, {'me': request.user})
...
#     """
#     message_list = Message.objects.inbox_for(request.user).exclude(favorite=request.user)
...
def outbox(request, template_name='messages/outbox.html'):
    ...
    Optional arguments:
        ``template_name``: name of the template to use.
    """
    message_list = Message.objects.outbox_for(request.user).exclude(favorite=request.user)
    return render_to_response(template_name, {
        'message_list': message_list,
    }, context_instance=RequestContext(request))
...
@login_required
def favorite(request, template_name='messages/favorite.html'):
    message_list = Message.objects.favorite_for(request.user)
    return render_to_response(template_name, {
        'message_list': message_list,
    }, context_instance=RequestContext(request))
...
@login_required
def draft(request, template_name='messages/draft.html'):
    draft_list = DraftMessage.objects.draft_for(request.user)
    return render_to_response(template_name, {
